...
static inline void unpack_buff_to_samples(uint8_t input[], const unsigned n_samples, const unsigned slot_size, uint32_t output[]){
  switch(slot_size){
    case 4:
...
#pragma unsafe arrays
static inline void pack_samples_to_buff(uint32_t input[], const unsigned n_samples, const unsigned slot_size, uint8_t output[]){
  switch(slot_size){
    case 4:
      for (int i = 0; i < n_samples; i++){
        unsigned base = i * 4;
